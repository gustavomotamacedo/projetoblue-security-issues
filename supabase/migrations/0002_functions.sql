...
-- Função: add_assets_to_association
...
-- Função: admin_update_user_profile
...
-- Função: check_asset_availability
...
-- Função: detect_association_inconsistencies
CREATE OR REPLACE FUNCTION detect_association_inconsistencies(
    OUT asset_id TEXT,
    OUT current_status_id BIGINT,
    OUT expected_status_id BIGINT,
    OUT issue_description TEXT,
    OUT corrected BOOLEAN
)
...
-- Função: has_minimum_role
CREATE OR REPLACE FUNCTION has_minimum_role(required_role user_role_enum)
RETURNS BOOLEAN
LANGUAGE plpgsql
AS $$
DECLARE
...
-- Função: is_afiliado
CREATE OR REPLACE FUNCTION is_afiliado()
RETURNS BOOLEAN
LANGUAGE sql
AS $$
...
-- Função: is_support_or_above
CREATE OR REPLACE FUNCTION is_support_or_above()
RETURNS BOOLEAN
LANGUAGE sql
AS $$
...
-- Função: is_user_self
CREATE OR REPLACE FUNCTION is_user_self(profile_id UUID)
RETURNS BOOLEAN
LANGUAGE sql
AS $$
...
-- Função: log_asset_soft_delete
CREATE OR REPLACE FUNCTION log_asset_soft_delete()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
-- Função: log_asset_status_change
...
-- Função: prevent_rented_association
CREATE OR REPLACE FUNCTION prevent_rented_association()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
-- Função: release_operation_lock
CREATE OR REPLACE FUNCTION release_operation_lock(
    p_lock_id UUID
)
RETURNS BOOLEAN
LANGUAGE plpgsql
...
-- Função: status_by_asset_type
CREATE OR REPLACE FUNCTION status_by_asset_type()
RETURNS TABLE (
    type TEXT,
    status TEXT,
    count BIGINT
...
-- Função: update_asset_rented_days
...
-- Função: update_generic_updated_at_column
CREATE OR REPLACE FUNCTION update_generic_updated_at_column()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
-- Função: update_profile_last_login
...
-- Função: validate_active_uuid
CREATE OR REPLACE FUNCTION validate_active_uuid()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
-- Função: validate_association_state
...
-- Função: validate_rented_days_integrity
CREATE OR REPLACE FUNCTION validate_rented_days_integrity(
    OUT asset_id TEXT,
    OUT current_rented_days INT,
    OUT calculated_days INT,
