...
// Função para mapear tipo de associação para status correto
export const getStatusIdFromAssociationType = (associationId: number): number => {
  switch (associationId) {
    case 1: return 2; // ALUGUEL -> status "ALUGADO" (id=2)
    case 2: return 3; // ASSINATURA -> status "ASSINATURA" (id=3)
    default: return 2; // Default para ALUGADO
  }
};

// Função para mapear tipo de associação para nome do status
export const getStatusNameFromAssociationType = (associationId: number): string => {
  switch (associationId) {
    case 1: return 'ALUGADO';
    case 2: return 'ASSINATURA';
    default: return 'ALUGADO';
  }
};
